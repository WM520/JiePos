...
@interface IBPayPassView () <UIKeyInput>

@end

@implementation IBPayPassView
...
- (void)insertText:(NSString *)text {
    ...
    if (self.saveStore.length < self.digitsNumber) {
...
- (void)deleteAll {
    if (self.saveStore.length > 0) {
        [self.saveStore deleteCharactersInRange:NSMakeRange(0, self.saveStore.length)];
        if ([self.delegate respondsToSelector:@selector(passwordDidChange:)]) {
            [self.delegate passwordDidChange:self];
        }
    }
