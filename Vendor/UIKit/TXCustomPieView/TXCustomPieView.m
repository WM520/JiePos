...
#import "TXCustomShapeLayer.h"

typedef void(^ClickBlock)(NSInteger clickIndex);

@interface TXCustomPieView (){
    //饼状图的实际半径
    CGFloat realRadius;
    //饼状图 array
...
@end
@implementation TXCustomPieView
...
- (CGFloat) widthForTextString:(NSString *)tStr height:(CGFloat)tHeight fontSize:(CGFloat)tSize {
    ...
    if (width > JPRealValue(230)) {
        width = JPRealValue(230);
    }
    return width;
}
...
- (void)loadCustomPieView{
    ...
    for ( int i = 0 ;i < self.segmentDataArray.count;i++) {
        ...
        //初始化Layer
        TXCustomShapeLayer *radiusLayer = [TXCustomShapeLayer layer];
        //设置layer的路径
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    for (TXCustomShapeLayer *shapeLayer in pieShapeLayerArray) {
...
- (void)clickPieView:(void(^)(NSInteger index))clickBlock{
    if (clickBlock) self.clickBlock =clickBlock;
}
#pragma mark - 圆心处理
- (void)loadPieCenter{
    CGFloat viewHeight = self.bounds.size.height;
