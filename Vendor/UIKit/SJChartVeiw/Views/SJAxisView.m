...
- (void)setupYMarkLabs {
    ...
    UILabel *unitLab = [self viewWithTag:1247];
    if (!unitLab) {
        unitLab = [[UILabel alloc] initWithFrame:(CGRect){JPRealValue(10), self.startPoint.y - YMARKLAB_HEIGHT - self.yScaleMarkLEN / 3.0, YMARKLAB_WIDTH, YMARKLAB_HEIGHT}];
        unitLab.text = @"（元）";
        unitLab.textColor = JP_NoticeText_Color;//[UIColor whiteColor];
    ...
    for (int i = 0; i < self.yMarkTitles.count; i ++) {
        ...
        if (!markLab) {
            markLab = [[UILabel alloc] initWithFrame:CGRectMake(0, self.startPoint.y - YMARKLAB_HEIGHT / 4.0 + i * self.yScaleMarkLEN - self.yScaleMarkLEN / 4.0, YMARKLAB_WIDTH, YMARKLAB_HEIGHT)];
            markLab.textAlignment = NSTextAlignmentRight;
            markLab.textColor = JP_NoticeText_Color;//[UIColor whiteColor];
            markLab.tag = i + 999;
            if (maxValue >= 100000) {
                markLab.font = [UIFont systemFontOfSize:10.0];
                unitLab.font = [UIFont systemFontOfSize:10.0];
            } else {
                markLab.font = [UIFont systemFontOfSize:12.0];
                unitLab.font = [UIFont systemFontOfSize:12.0];
            }
...
- (void)setupXMarkLabs {
    ...
    for (int i = 0; i < self.xMarkTitles.count; i ++) {
        ...
        if (!markLab) {
            ...
            NSDate *date = [NSDate dateFromString:self.xMarkTitles[i] withFormat:@"yyyy/MM/dd"];
            NSString *dateStr = [NSDate stringFromDate:date withFormat:@"MM/dd"];
            NSInteger weekends = [date dayOfWeek];
            if (weekends == 1) {
                markLab.text = dateStr;
            }
            [self addSubview:markLab];
        }
    }
...
- (void)drawYAxsiLine {
    ...
    yAxisLayer.strokeColor = JP_NoticeText_Color.CGColor;//[UIColor whiteColor].CGColor;
...
- (void)drawXAxsiLine {
    ...
    xAxisLayer.strokeColor = JP_NoticeText_Color.CGColor;//[UIColor whiteColor].CGColor;
