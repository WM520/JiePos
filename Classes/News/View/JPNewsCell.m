...
- (void)handleUserInterce {
    ...
    if (!self.titleLab) {
        ...
        self.titleLab.text = [NSString strmethodComma:@"+0"];
    ...
    if (!self.totalAmtLab) {
        ...
        self.totalAmtLab.textColor = JP_NoticeText_Color;
        self.totalAmtLab.font = JP_DefaultsFont;
    ...
    if (!self.couponAmtLab) {
        ...
        self.couponAmtLab.font = JP_DefaultsFont;
        self.couponAmtLab.textColor = JP_NoticeText_Color;
    ...
    if (!self.unreadLab) {
        self.unreadLab = [UILabel new];
        self.unreadLab.font = [UIFont systemFontOfSize:JPRealValue(20)];
        self.unreadLab.text = @"未读";
        self.unreadLab.textColor = [UIColor whiteColor];
        self.unreadLab.textAlignment = NSTextAlignmentCenter;
        self.unreadLab.backgroundColor = JP_NoticeRedColor;
        self.unreadLab.layer.cornerRadius = JPRealValue(5);
...
#pragma mark - Setter
- (void)setNewsModel:(JPNewsModel *)newsModel {
...
//    self.totalAmt.attributedText = attrStr;
    self.totalAmt.text = [NSString stringWithFormat:@"%@ 元", totalAmt];
...
//    self.couponAmt.attributedText = attrStr2;
    self.couponAmt.text = [NSString stringWithFormat:@"%@ 元", couponAmt];
    ...
    self.unreadLab.hidden = !newsModel.unread;
}
...
- (void)layoutSubviews {
    ...
    [self.unreadLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(weakSelf.indicatorView.mas_left).offset(JPRealValue(-20));
        make.size.mas_equalTo(CGSizeMake(JPRealValue(58), JPRealValue(26)));
    }];
}
