...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle clickIndexBlock:(LXAlertClickIndexBlock)block{
    if(self=[super init]){
        ...
        _imageView.image = [UIImage imageNamed:@"carcertification_alert_picture"];
        _imageView.frame = CGRectMake(messageLabSpace, 20 + _titleLab.frame.size.height, AlertView_W-messageLabSpace*2, 70);
        ...
        _messageLab.frame=CGRectMake(messageLabSpace, 30 + _titleLab.frame.size.height, AlertView_W-messageLabSpace*2, endMessageLabH);
        
        //计算_alertView的高度
        _alertView.frame=CGRectMake(0, 0, AlertView_W, height + _messageLab.frame.size.height +LXATitle_H+LXABtn_H+40);
...
//        [_alertView addSubview:_imageView];
        ...
        if (otherBtnTitle) {
            ...
            [_otherBtn setBackgroundImage:[UIImage imageNamed:@"jp_merchants_through_bg1"] forState:UIControlStateNormal];
        ...
        if (cancelTitle && !otherBtnTitle) {
            ...
            _otherBtn.tag=0;
            _otherBtn.frame=CGRectMake(btnLeftSpace, btn_y, AlertView_W-btnLeftSpace*2, 40);
        }else if (cancelTitle && otherBtnTitle){
            _cancelBtn.tag=0;
            _otherBtn.tag=1;
            CGFloat btnSpace = 20;//两个btn之间的间距
            CGFloat btn_w =(AlertView_W-btnLeftSpace*2-btnSpace)/2;
            _cancelBtn.frame=CGRectMake(btnLeftSpace, btn_y + 20, btn_w, LXABtn_H);
            _otherBtn.frame=CGRectMake(_alertView.frame.size.width-btn_w-btnLeftSpace, btn_y + 20, btn_w, LXABtn_H);
        }
...
- (instancetype)initWithTitle:(NSString *)title imageName:(NSString *)imageName message:(NSString *)message commitBtnTitle:(NSString *)commitTitle clickIndexBlock:(LXAlertClickIndexBlock)block
{
    self = [super init];
    if (self) {
        self.frame=MainScreenRect;
        ...
        CGFloat btnLeftSpace = 40;
        CGFloat btn_y = _alertView.frame.size.height-40;
        _otherBtn=[UIButton buttonWithType:UIButtonTypeCustom];
        [_otherBtn setTitle:commitTitle forState:UIControlStateNormal];
        [_otherBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _otherBtn.titleLabel.font=LXADBtnTitleFont;
        _otherBtn.layer.cornerRadius=3;
        _otherBtn.layer.masksToBounds=YES;
