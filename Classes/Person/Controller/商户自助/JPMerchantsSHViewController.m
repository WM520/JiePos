...
@property (nonatomic, strong) JPSelfHelpDataModel *dataModel;
...
- (void)createRequestWithType:(NSString *)type {
    [IBProgressHUD loadingWithStatus:@"信息同步中，请稍后！"];
    weakSelf_declare;
    
    [IBPersonRequest getCertificateWithAccount:[JPUserEntity sharedUserEntity].account type:type callback:^(NSString *code, NSString *msg, id resp) {
        JPLog(@"%@ - %@ - %@", code, msg, resp);
        if (code.integerValue == 0) {
            ...
            if ([resp isKindOfClass:[NSArray class]]) {
                ...
                [response setObject:resp forKey:@"data"];
                
                weakSelf.docModel = [JPDocumentsModel yy_modelWithJSON:response];
...
- (void)createRequest {
    ...
    [IBPersonRequest getMerchantInfoWithAccount:[JPUserEntity sharedUserEntity].account callback:^(NSString *code, NSString *msg, id resp) {
        
         id obj = [IBAnalysis analysisWithEncryptString:resp privateKey:[JPUserEntity sharedUserEntity].privateKey];
        JPLog(@"%@ - %@ - %@", code, msg, obj);
        if (code.integerValue == 0) {
            weakSelf.dataModel = [JPSelfHelpDataModel yy_modelWithJSON:obj];
            weakSelf.isEnterprise = weakSelf.dataModel.merchantCategory == 1;
            if (weakSelf.dataModel.merchantCategory == 2) {
                weakSelf.hasLicence = weakSelf.dataModel.certificateImgType == 1;
            }
            weakSelf.isPublic = weakSelf.dataModel.accountType == 1;
        } else {
            [IBProgressHUD showInfoWithStatus:msg];
    ...
    if (self.applyProgress == JPApplyProgressNotThrough) {
        ...
        [IBPersonRequest openCityWithAccount:[JPUserEntity sharedUserEntity].account parent:@"1" level:@"1" callback:^(NSString *code, NSString *msg, id resp) {
            JPLog(@"%@ - %@ - %@", code, msg, resp);
            if (code.integerValue == 0) {
                if ([resp isKindOfClass:[NSArray class]]) {
        ...
        [IBPersonRequest bankNameWithAccount:[JPUserEntity sharedUserEntity].account province:nil city:nil bankName:nil callback:^(NSString *code, NSString *msg, id resp) {
            JPLog(@"%@ - %@ - %@", code, msg, resp);
            if (code.integerValue == 0) {
                if ([resp isKindOfClass:[NSArray class]]) {
...
- (UITableView *)baseCtntView {
    if (!_baseCtntView) {
        ...
        _baseCtntView.estimatedSectionHeaderHeight = 0;
        _baseCtntView.estimatedSectionFooterHeight = 0;
        
        [_baseCtntView registerClass:[JPRemarkCell class] forCellReuseIdentifier:@"remarkCellReuseIdentifier"];
    }
...
- (UITableView *)billingCtntView {
    if (!_billingCtntView) {
        ...
        _billingCtntView.estimatedSectionHeaderHeight = 0;
        _billingCtntView.estimatedSectionFooterHeight = 0;
    }
    ...
    if (tableView == self.baseCtntView) {
        if (indexPath.section == 0) {
            ...
            if (indexPath.row == 0) {
                ...
                cell.jp_cateSelectBlock = ^(NSInteger tag) {
                    ...
                    if (weakSelf.applyProgress == JPApplyProgressThrough || weakSelf.applyProgress == JPApplyProgressApplying) {
                        [IBProgressHUD showInfoWithStatus:weakSelf.reviewStatus];
                    } else {
                ...
                cell.inputField.text = self.dataModel.merchantName;
                return cell;
                ...
                cell.title = @"商户简称";
                cell.inputField.placeholder = @"不超过15个汉字";
                cell.inputField.enabled = self.applyProgress == JPApplyProgressNotThrough;
                cell.inputField.text = self.dataModel.merchantShortName;
                return cell;
                ...
                cell.title = @"注册地址";
                cell.textLab.text = [NSString stringWithFormat:@"%@%@%@", self.dataModel.registerProvinceName, self.dataModel.registerCityName, self.dataModel.registerDistrictName];
                ...
                cell.inputField.text = self.dataModel.registerAddress;
                cell.inputField.enabled = self.applyProgress == JPApplyProgressNotThrough;
            ...
            if (indexPath.row == 0) {
                ...
                cell.title = @"行业类型";
                cell.textLab.text = self.dataModel.industryType;
                cell.textLab.textColor = JP_Content_Color;
                return cell;
                ...
                cell.textLab.text = self.dataModel.industryNo;
                cell.textLab.textColor = JP_Content_Color;
                return cell;
            ...
            if (indexPath.row == 0) {
                ...
                cell.inputField.enabled = self.applyProgress == JPApplyProgressNotThrough;
                cell.inputField.text = self.dataModel.legalPersonName;
                return cell;
                ...
                cell.title = @"用户名";
                cell.inputField.text = self.dataModel.userName;
                ...
                cell.inputField.enabled = self.applyProgress == JPApplyProgressNotThrough;
                cell.inputField.text = self.dataModel.accountIdcard;
            ...
            // !!!: 备注
            JPRemarkCell *cell = [tableView dequeueReusableCellWithIdentifier:@"remarkCellReuseIdentifier" forIndexPath:indexPath];
            cell.canEdit = self.applyProgress == JPApplyProgressNotThrough;
            cell.remarkString = self.dataModel.remark;
//            self.remark = cell.remarkString;
            return cell;
        }
    } else {
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                ...
                cell.title = @"开户地";
                cell.textLab.text = self.dataModel.accountProvinceName;
                cell.textLab.textColor = JP_Content_Color;
                return cell;
                ...
                cell.textLab.text = self.dataModel.accountCityName;
                cell.textLab.textColor = JP_Content_Color;
                return cell;
            ...
            if (indexPath.row == 0) {
                ...
                cell.title = @"银行名称";
                cell.textLab.text = self.dataModel.accountBankName;
                cell.textLab.textColor = JP_Content_Color;
                return cell;
                ...
                cell.title = @"支行名称";
                cell.textLab.text = self.dataModel.accountBankBranchName;
                cell.textLab.textColor = JP_Content_Color;
                return cell;
...
//                cell.inputField.enabled = self.applyProgress == JPApplyProgressNotThrough;
//                cell.inputField.text = self.dataModel.account;
//                cell.inputField.keyboardType = UIKeyboardTypeNumberPad;
//                cell.inputField.tag = 444444;
//                cell.inputField.delegate = self;
//                return cell;
                
                JPTxtViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"2cell22"];
                if (!cell) {
                    cell = [[JPTxtViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"2cell22"];
                }
                cell.title = @"开户银行账号";
//                cell.txtView.placeholderText = @"请输入开户银行账号";
                cell.txtView.placeholder = @"请输入开户银行账号";
                cell.txtView.userInteractionEnabled = self.applyProgress == JPApplyProgressNotThrough;
                cell.txtView.text = self.dataModel.account;
                cell.txtView.keyboardType = UIKeyboardTypeNumberPad;
//                cell.txtView.tag = 444444;
                return cell;
                ...
                cell.inputField.enabled = self.applyProgress == JPApplyProgressNotThrough;
                cell.inputField.text = self.dataModel.accountName;
                return cell;
                ...
                cell.inputField.enabled = self.applyProgress == JPApplyProgressNotThrough;
                cell.inputField.text = self.dataModel.contactMobilePhone;
                cell.inputField.keyboardType = UIKeyboardTypeNumberPad;
...
//        [IBProgressHUD showInfoWithStatus:@"审核已通过，无法修改！"];
        [IBProgressHUD showInfoWithStatus:self.reviewStatus];
    } else if (self.applyProgress == JPApplyProgressApplying) {
//        [IBProgressHUD showInfoWithStatus:@"审核中，请耐心等待！"];
        [IBProgressHUD showInfoWithStatus:self.reviewStatus];
    } else {
        //  只有审核未通过才可修改
        if (tableView == self.baseCtntView) {
            if (indexPath.section == 0) {
                if (indexPath.row == 4) {
                    [IBProgressHUD showInfoWithStatus:@"注册地址无法修改！"];
                }
            } else if (indexPath.section == 1) {
                [IBProgressHUD showInfoWithStatus:@"行业类型无法修改！"];
            } else if (indexPath.section == 2) {
                if (indexPath.row == 1) {
                    [IBProgressHUD showInfoWithStatus:@"用户名不可修改！"];
                }
            }
            ...
            if (self.isSelect == false) {
                ...
                if (indexPath.section == 0) {
                    if (indexPath.row == 1) {
                        ...
                        if (self.provinces.count > 0) {
                            ...
                            if (data.count > 0) {
                                ...
                                [IBProgressHUD showInfoWithStatus:JPServerNot00];
                            }
                        } else {
                            [IBProgressHUD showInfoWithStatus:JPServerNoFinish];
                        }
                    } else if (indexPath.row == 2) {
                        //  选择开户市
                        ...
                        if (parent == nil) {
                            [IBProgressHUD showInfoWithStatus:@"请选择省/直辖市/自治区！"];
                            return;
                        }
                        weakSelf_declare;
                        [IBProgressHUD loadingWithStatus:@"信息获取中，请稍后..."];
                        [self canDoSthInView:NO];
                        [IBPersonRequest openCityWithAccount:[JPUserEntity sharedUserEntity].account parent:parent level:@"2" callback:^(NSString *code, NSString *msg, id resp) {
                            ...
                            if (code.integerValue == 0) {
                                ...
                                if ([resp isKindOfClass:[NSArray class]]) {
                                    ...
                                    if (data.count > 0) {
                                        ...
                                        [IBProgressHUD showInfoWithStatus:JPBranchBank];
                    ...
                    if (indexPath.row == 0) {
                        ...
                        if (provinceCode == nil) {
                            provinceCode = self.dataModel.accountProvinceCode;
                        }
                        ...
                        if (cityCode == nil) {
                            cityCode = self.dataModel.accountCityCode;
                        }
                        weakSelf_declare;
                        [IBProgressHUD loadingWithStatus:@"信息获取中，请稍后..."];
                        [self canDoSthInView:NO];
                        
                        [IBPersonRequest bankNameWithAccount:[JPUserEntity sharedUserEntity].account province:provinceCode city:cityCode bankName:bankNameCell.textLab.text callback:^(NSString *code, NSString *msg, id resp) {
                            if (code.integerValue == 0) {
                                //  成功
                                [IBProgressHUD dismiss];
                                if ([resp isKindOfClass:[NSArray class]]) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.baseCtntView) {
        ...
        if (myIndexPath == indexPath) {
            ...
            return indexPath.section == 3 ? JPRealValue(240) : JPRealValue(100);
        }
    } else if (tableView == self.billingCtntView) {
        NSIndexPath *accountIndex = [NSIndexPath indexPathForRow:2 inSection:1];
        return indexPath == accountIndex ? JPRealValue(120) : JPRealValue(100);
    } else {
        return 0.01;
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    weakSelf_declare;
    JPCredentialsCell *blockCell = (JPCredentialsCell *)cell;
    cell.credentialsDeleteImageBlock = ^(JPCredentialsCell *item) {
        JPLog(@"delete");
        weakSelf.imageHasChange = YES;
        
        //  遍历商户自助信息
        for (JPCertificateData *certificateData in self.dataModel.imgs) {
            for (JPImageModel *imageModel in certificateData.imgList) {
                if ([blockCell.imgCode isEqualToString:imageModel.imgDesc]) {
                    imageModel.url = @"";
                }
            }
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.applyProgress == JPApplyProgressThrough) {
        ...
        if (cell.hasImage) {
            ...
            [IBProgressHUD showInfoWithStatus:@"暂无图片"];
        }
    } else if (self.applyProgress == JPApplyProgressApplying) {
...
- (void)rightItemClicked:(UIBarButtonItem *)sender {
    //  提交修改
    BOOL enterprise = self.dataModel.certificateImgType == 0;
    BOOL haveLicence = self.dataModel.certificateImgType == 1;
    BOOL public = self.dataModel.accountType == 1;
    ...
    // !!!: 备注
    JPRemarkCell *remarkCell = [self.baseCtntView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:3]];
    ...
    // !!!: 开户银行账号
    JPTxtViewCell *accountCell = [self.billingCtntView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:1]];
    // !!!: 开户账号名称
    ...
    if (merchantNameCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"商户名称为空！"];
        return;
    }
    if (merchantNameCell.inputField.text.length > 32) {
        [IBProgressHUD showInfoWithStatus:@"商户名称不超过32个汉字！"];
        return;
    }
    ...
    if (merchantShortNameCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"商户简称为空！"];
        return;
    }
    if (merchantShortNameCell.inputField.text.length > 15) {
        [IBProgressHUD showInfoWithStatus:@"商户简称不超过15个汉字！"];
        return;
    }
    ...
    if (detailAddressCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"详细地址为空！"];
        return;
    }
    
    if (legalPersonCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"法人姓名为空！"];
        return;
    }
    if (legalPersonCell.inputField.text.length < 2) {
        [IBProgressHUD showInfoWithStatus:@"法人姓名为2-10个汉字！"];
        return;
    }
    ...
    if (IDCardNumberCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"身份证号为空！"];
        return;
    }
    if (![NSString accurateVerifyIDCardNumber:IDCardNumberCell.inputField.text]) {
        [IBProgressHUD showInfoWithStatus:@"身份证号输入有误！"];
        return;
    }
    
    if ([accountProvinceCell.textLab.text isEqualToString:@"--请选择省/直辖市/自治区！--"]) {
        [IBProgressHUD showInfoWithStatus:@"请选择开户地（省）！"];
        return;
    }
    
    if ([accountCityCell.textLab.text isEqualToString:@"--请选择开户城市（市）--"]) {
        [IBProgressHUD showInfoWithStatus:@"请选择开户地（市）！"];
        return;
    }
    
    if ([bankNameCell.textLab.text isEqualToString:@"--请选择开户行--"]) {
        [IBProgressHUD showInfoWithStatus:@"请选择银行名称！"];
        return;
    }
    
    if ([branchBankNameCell.textLab.text isEqualToString:@"请选择支行名称"]) {
        [IBProgressHUD showInfoWithStatus:@"请选择支行名称！"];
        return;
    }
    
    if (accountCell.txtView.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"开户银行账号为空！"];
        return;
    }
    if (accountCell.txtView.text.length > 40) {
        [IBProgressHUD showInfoWithStatus:@"开户银行账号不超过40位！"];
        return;
    }
    if (accountNameCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"开户账号名称为空！"];
        return;
    }
    if (accountNameCell.inputField.text.length > 32) {
        [IBProgressHUD showInfoWithStatus:@"开户账号名称不超过32个汉字！"];
        return;
    }
    ...
    if (mobileCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"预留手机号为空！"];
        return;
    }
    //  格式化后有空格 转成正常的字符串
    //  开户账号
    NSString *account = accountCell.txtView.text;//[UITextField numberToNormalNumTextField:accountCell.txtView];
    if (!account || account.length <= 0) {
        account = self.dataModel.account;
    }
    ...
    if (![mobile isPhone]) {
        [IBProgressHUD showInfoWithStatus:@"手机号输入有误！"];
        return;
    }
    
    if (!mobile || mobile.length <= 0) {
        mobile = self.dataModel.contactMobilePhone;
    }
    ...
    if (self.isPublic) {
        if (![accountNameCell.inputField.text isEqualToString:merchantNameCell.inputField.text]) {
            [IBProgressHUD showInfoWithStatus:@"开户账号名称和商户名称须一致！"];
            return;
        }
    }
    ...
    if (accountProvinceCode == nil) {
        accountProvinceCode = self.dataModel.accountProvinceCode;
    }
    ...
    if (accountCityCode == nil) {
        accountCityCode = self.dataModel.accountCityCode;
    }
    ...
    if (accountBankNameId == nil) {
        accountBankNameId = self.dataModel.accountBankName;
    }
    ...
    if (alliedBankCode == nil) {
        alliedBankCode = self.dataModel.accountBankBranchCode;
        accountBankBranchName =  self.dataModel.accountBankBranchName;
    }
    
    //  开户银行名称
    NSString *accountName = accountNameCell.inputField.text;
    if (!accountName || accountName.length <= 0) {
        accountName = self.dataModel.accountName;
    }
    
//    JPLog(@"%d - %d - %d - %@ - %@ - %@ - %@", self.isEnterprise, self.hasLicence, self.isPublic, merchantCategory, certificateImgType, accountType, self.remark);
    JPLog(@"remark：%@ - %@", remarkCell.remarkString, self.dataModel.remark);
...
//    JPLog(@"商户自助 - %d - %d - %@ - %@ - %@ - %@ - %@\n%d - %@ - %@ - %@ - %@ - %@ - %@ - %@ - %@ - %@ - %@", self.isEnterprise, self.hasLicence, merchantNameCell.inputField.text, merchantShortNameCell.inputField.text, self.selfHelpModel.data.registerAddress, self.selfHelpModel.data.industryType, self.selfHelpModel.data.industryNo, self.isPublic, legalPersonCell.inputField.text, userNameCell.inputField.text, IDCardNumberCell.inputField.text, accountProvinceCell.textLab.text, accountCityCell.textLab.text, bankNameCell.textLab.text, branchBankNameCell.textLab.text, accountCell.inputField.text, accountNameCell.inputField.text, mobileCell.inputField.text);
    ...
    for (NSInteger j = 0; j < self.dataSource.count; j ++) {
        ...
        if (!cell.hasImage && cell.isNeed) {
            [IBProgressHUD showInfoWithStatus:@"证件资料缺失，请补齐资料后重新提交！"];
            return;
        }
    }
    
    // !!!: 数据是否发生过变化
    BOOL valueChanged = (enterprise != self.isEnterprise) || (haveLicence != self.hasLicence) || (public != self.isPublic) || (![merchantNameCell.inputField.text isEqualToString:self.dataModel.merchantName]) || (![merchantShortNameCell.inputField.text isEqualToString:self.dataModel.merchantShortName]) || (![detailAddressCell.inputField.text isEqualToString:self.dataModel.registerAddress]) || (![legalPersonCell.inputField.text isEqualToString:self.dataModel.legalPersonName]) || (![NSString accurateVerifyIDCardNumber:IDCardNumberCell.inputField.text]) || (![IDCardNumberCell.inputField.text isEqualToString:self.dataModel.accountIdcard]) || (![accountProvinceCell.textLab.text isEqualToString:self.dataModel.accountProvinceName]) || (![accountCityCell.textLab.text isEqualToString:self.dataModel.accountCityName]) || (![bankNameCell.textLab.text isEqualToString:self.dataModel.accountBankName]) || (![branchBankNameCell.textLab.text isEqualToString:self.dataModel.accountBankBranchName]) || (![accountNameCell.inputField.text isEqualToString:self.dataModel.accountName]) || (![account isEqualToString:self.dataModel.account]) || (![mobile isEqualToString:self.dataModel.contactMobilePhone]) || (![merchantCategory isEqualToString:[NSString stringWithFormat:@"%ld", (long)self.dataModel.merchantCategory]]) || (![certificateImgType isEqualToString:[NSString stringWithFormat:@"%ld", (long)self.dataModel.certificateImgType]]) || (![accountType isEqualToString:[NSString stringWithFormat:@"%ld", (long)self.dataModel.accountType]]) || (![legalPerson isEqualToString:self.dataModel.legalPersonName]) || (![IDCardNumber isEqualToString:self.dataModel.accountIdcard]) || (![accountProvinceCell.textLab.text isEqualToString:self.dataModel.accountProvinceName]) || (![accountCityCell.textLab.text isEqualToString:self.dataModel.accountCityName]) || (![bankNameCell.textLab.text isEqualToString:self.dataModel.accountBankName]) || (![branchBankNameCell.textLab.text isEqualToString:self.dataModel.accountBankBranchName]) || (![accountName isEqualToString:self.dataModel.accountName]) || self.imageHasChange || (![remarkCell.remarkString isEqualToString:self.dataModel.remark]);
    
    JPLog(@"valueChanged：%d", valueChanged);
//    return;
    if (!valueChanged) {
        [IBProgressHUD showInfoWithStatus:@"您还没有作任何修改！"];
        return;
    }
    
    [IBProgressHUD loadingWithStatus:@"进件资料提交中，请稍后..."];
    [self canDoSthInView:NO];
    ...
    if (![merchantNameCell.inputField.text isEqualToString:self.dataModel.merchantName]) {
        dispatch_group_enter(group);
        
        [IBPersonRequest checkUserInfoAccount:[JPUserEntity sharedUserEntity].account merchantId:[JPUserEntity sharedUserEntity].merchantId isUserName:false content:merchantNameCell.inputField.text qrCodeId:nil callback:^(NSString *code, NSString *msg, id resp) {
            ...
            if (code.integerValue == 0) {
                ...
                if ([resp isKindOfClass:[NSDictionary class]]) {
                    ...
                    if (isExist) {
                        [IBProgressHUD showInfoWithStatus:@"商户名已存在！"];
                        [weakSelf canDoSthInView:YES];
                        return;
                    }
                }
    ...
    for (NSInteger i = 0; i < self.dataSource.count; i ++) {
        ...
        if (contentOffset_x >= kScreenWidth * 2) {
            ...
            JPLog(@"Cell%ld  -  %d", (long)i, cell.valueHasChange);
                    ...
                    for (JPCertificateData *docData in weakSelf.docModel.data) {
                        for (JPImageModel *imageModel in docData.imgList) {
                            if ([imageModel.imgDesc isEqualToString:cell.imgCode]) {
                                ...
                                JPCertificateData *defDataModel = weakSelf.dataModel.imgs[dataIndex];
...
//            [weakSelf.docModel setData:weakSelf.credentModel.data.imgs];
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        ...
        if (dic.count <= 0) {
            NSDictionary *Dic = [JPTransferMJ getObjectData:weakSelf.dataModel];
            dataArray = Dic[@"imgs"];
        }
//        JPLog(@"dataArray - %@", dataArray);
        
        [MobClick event:@"merchant_save"];
        
        [IBPersonRequest commitMerchantInfoWithMerchantCategory:merchantCategory certificateImgType:certificateImgType merchantName:merchantNameCell.inputField.text merchantShortName:merchantShortNameCell.inputField.text registerProvinceCode:weakSelf.dataModel.registerProvinceCode registerCityCode:weakSelf.dataModel.registerCityCode registerDistrictCode:weakSelf.dataModel.registerDistrictCode registerAddress:detailAddress industryType:weakSelf.dataModel.industryType mcc:weakSelf.dataModel.mcc industryNo:weakSelf.dataModel.industryNo legalPersonName:legalPerson username:weakSelf.dataModel.userName accountIdcard:IDCardNumber accountType:accountType accountProvinceCode:accountProvinceCode accountCityCode:accountCityCode accountBankNameId:accountBankNameId alliedBankCode:alliedBankCode accountBankBranchName:accountBankBranchName account:account accountName:accountName contactMobilePhone:mobile qrcodeId:nil merchantId:[NSString stringWithFormat:@"%ld", (long)weakSelf.dataModel.merchantId] imgs:dataArray remark:remarkCell.remarkString callback:^(NSString *code, NSString *msg, id resp) {
