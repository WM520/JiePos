                    ...
                    if ([code isEqualToString:@"00"]) {
                        if ([resp isKindOfClass:[NSArray class]]) {
                            ...
                            for (NSDictionary *dic in arr) {
                                ...
                                [data addObject:countyModel.name];
                            }
                            if (data.count > 0) {
                                [KYSNormalPickerView KYSShowWithDataArray:@[data] completeBlock:^(NSArray *selectArray) {
                                    NSInteger index = [[selectArray lastObject] integerValue];
...
//            [IBPersonRequest industryWithAccount:[JPUserEntity sharedUserEntity].account name:model.name callback:^(NSString *code, NSString *msg, id resp) {
//                JPLog(@"%@ - %@ - %@", code, msg, resp);
...
//                        //  把新数据加入到数组
//                        for (NSDictionary *dic in arr) {
//                            JPIndustryModel *indusModel = [JPIndustryModel yy_modelWithDictionary:dic];
//                            [weakSelf.indusNoList addObject:indusModel];
//                        }
//
//                        NSMutableArray *data = @[].mutableCopy;
//                        for (JPIndustryModel *model in weakSelf.indusNoList) {
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        CGFloat height = is_iPhone_X ? 44 : 20;
