...
@interface JPBillingInfoViewController () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, YYTextViewDelegate>
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            cell.jp_cateSelectBlock = ^(NSInteger tag) {
                ...
                [tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:3 inSection:1]] withRowAnimation:UITableViewRowAnimationAutomatic];
            };
            return cell;
        ...
        if (indexPath.row == 0) {
            ...
            return cell;
//            JPTxtViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell22"];
//            if (!cell) {
//                cell = [[JPTxtViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell22"];
//            }
//            cell.title = @"开户银行账号";
//            cell.txtView.placeholderText = @"请输入开户银行账号";
//            cell.txtView.text = [JPUserInfoHelper objectForKey:@"account"];
...
//            cell.inputField.enabled = !self.isPublic;
            return cell;
    ...
    if (self.isSelect == false) {
        ...
        if (indexPath.section == 0) {
            if (indexPath.row == 1) {
                ...
                //            JPLog(@"%@", provinceCell.textLab.text);
                ...
                if (![provinceCell.textLab.text isEqualToString:@"--请选择省/直辖市/自治区--"]) {
                    ...
                    weakSelf_declare;
                    [IBPersonRequest openCityWithAccount:[JPUserEntity sharedUserEntity].account parent:provinceCode level:@"2" callback:^(NSString *code, NSString *msg, id resp) {
                        if (code.integerValue == 0) {
                            if ([resp isKindOfClass:[NSArray class]]) {
                                NSArray *arr = (NSArray *)resp;
                                [weakSelf.cities removeAllObjects];
                                ...
                                if (weakSelf.cities.count <= 0) {
                                    [IBProgressHUD showInfoWithStatus:@"暂无开户地区信息，请联系客户经理或者更换银行卡！"];
                                    return;
                                }
                    ...
                    [IBProgressHUD showInfoWithStatus:@"请先选择省/直辖市/自治区！"];
                }
            }
        } else {
            if (indexPath.row == 0) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSIndexPath *accountIndexPath = [NSIndexPath indexPathForRow:2 inSection:1];
    return indexPath == accountIndexPath ? JPRealValue(120) : JPRealValue(100);
}

- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return section == 0 ? JPRealValue(340) : 0.01;//JPRealValue(20);
}
    ...
    [JPNetworkUtils netWorkState:^(NSInteger netState) {
        switch (netState) {
            ...
            case 2: {
                ...
                break;
            default: {
                [IBProgressHUD showInfoWithStatus:JPServerNoNet];
            }
                break;
        }
    }];
...
- (void)varifyAndNext {
    ...
    // TODO: 判空验证
    if ([provinceCell.textLab.text isEqualToString:@"--请选择省/直辖市/自治区--"]) {
        [IBProgressHUD showInfoWithStatus:@"请选择开户省！"];
        return;
    }
    if ([cityCell.textLab.text isEqualToString:@"--请选择开户城市（市）--"]) {
        [IBProgressHUD showInfoWithStatus:@"请选择开户市！"];
        return;
    }
    if ([bankNameCell.textLab.text isEqualToString:@"--请选择开户行--"]) {
        [IBProgressHUD showInfoWithStatus:@"请选择开户行名称！"];
        return;
    }
    if ([branchBankNameCell.textLab.text isEqualToString:@"请选择支行名称"]) {
    ...
    if (accountCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"请输入开户银行账号！"];
        return;
    }
    if (accountUserNameCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"请输入开户银行名称！"];
        return;
    }
    ...
    if (phoneCell.inputField.text.length <= 0) {
        [IBProgressHUD showInfoWithStatus:@"请输入预留手机号！"];
        return;
    }
    ...
    if (accountCityCode.length <= 0 || !accountCityCode) {
        accountCityCode = [JPUserInfoHelper objectForKey:@"accountCityCode"];
    }
    certificateInfoVC.accountCityName = accountCityCode;
