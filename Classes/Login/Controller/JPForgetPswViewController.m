...
- (void)handleGetCode:(UIButton *)codeButton {
    ...
    //  获取验证码
    [IBLoginRequest getCodeWithAccount:userField.text phone:phone isCashPass:false callback:^(NSString *code, NSString *msg, id resp) {
...
- (void)handleNext {
    ...
    weakSelf_declare;
    [IBLoginRequest resetPswWithAccount:userField.text code:codeField.text phone:phone isCashPass:false callback:^(NSString *code, NSString *msg, id resp) {
        JPLog(@"修改密码 - %@", resp);
        if (code.integerValue == 0) {
            //  已发送密码至该号码，注意查收（重置成功）
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        { //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [codeButton setTitleColor:JP_Content_Color forState:UIControlStateNormal];
