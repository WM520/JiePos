    ...
    [IBPersonRequest getCertificateWithAccount:[JPUserEntity sharedUserEntity].account type:type callback:^(NSString *code, NSString *msg, id resp) {
        if (code.integerValue == 0) {
            ...
            if ([resp isKindOfClass:[NSArray class]]) {
                ...
                weakSelf.documentsModel = [JPDocumentsModel yy_modelWithJSON:response];
...
//                    [IBPersonRequest uploadImageWithAccount:[JPUserEntity sharedUserEntity].account image:cell.bgView.image isUpdate:false checkContent:weakSelf.qrcodeid tagStr:cell.imgCode progress:nil callback:^(NSString *code, NSString *msg, id resp) {
//                        JPLog(@"%@ - %@ - %@", code, msg, resp);
//                        if (code.integerValue == 0) {
